...
export function RightPanel({ selectedFile, isOpen, onClose }: RightPanelProps) {
  const [activeTab, setActiveTab] = useState<'recent' | 'activity' | 'details'>('recent');

  const recentFiles = [
    { name: 'genome_seq_01.fasta', sequences: 245, time: '2 hours ago' },
    { name: 'protein_analysis.fa', sequences: 128, time: '5 hours ago' },
    { name: 'mitochondrial_dna.fasta', sequences: 89, time: '1 day ago' },
    { name: 'viral_genome.fa', sequences: 412, time: '2 days ago' },
  ];
          ...
          {activeTab === 'recent' && (
            <>
              {recentFiles.map((file, index) => (
                <div
                  key={index}
                  className="p-4 rounded-lg bg-purple-50/50 dark:bg-purple-900/20 border border-purple-100/50 dark:border-purple-800/50 hover:shadow-sm transition-all cursor-pointer"
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{file.sequences} sequences</p>
                      <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">{file.time}</p>
                    </div>
                  </div>
                </div>
              ))}
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 truncate">{activity.file}</p>
            ...
            <div className="space-y-4">
              {selectedFile ? (
                <>
                  <div className="p-4 rounded-lg bg-purple-50/50 dark:bg-purple-900/20 border border-purple-100/50 dark:border-purple-800/50">
                    <div className="flex items-center gap-2 mb-3">
                      <Icons.Info className="w-4 h-4 text-purple-500 dark:text-purple-400" />
                      <p className="text-sm text-gray-700 dark:text-gray-300">Selected File</p>
                    </div>
                    <p className="text-sm text-gray-900 dark:text-white">{selectedFile.name}</p>
                    <div className="mt-3 space-y-2">
                      <div className="flex justify-between text-xs">
                        <span className="text-gray-500 dark:text-gray-400">Sequences:</span>
                        <span className="text-gray-900 dark:text-white">{selectedFile.sequences}</span>
                      </div>
                      <div className="flex justify-between text-xs">
                        <span className="text-gray-500 dark:text-gray-400">Size:</span>
                        <span className="text-gray-900 dark:text-white">{selectedFile.size}</span>
                      </div>
                      <div className="flex justify-between text-xs">
                        <span className="text-gray-500 dark:text-gray-400">Uploaded:</span>
                        <span className="text-gray-900 dark:text-white">{selectedFile.date}</span>
                      </div>
                    </div>
                  </div>
                </>
                ...
                <div className="p-8 text-center">
                  <Icons.FileText className="w-12 h-12 text-gray-300 dark:text-gray-600 mx-auto mb-3" />
                  <p className="text-sm text-gray-500 dark:text-gray-400">No file selected</p>
                  <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">Select a file to view details</p>
                </div>
              )}
            </div>
          )}
