...
type ViewType = 'upload' | 'recent' | 'metadata' | 'report';

export default function App() {
  const [activeView, setActiveView] = useState<ViewType>('upload');
  const [selectedFile, setSelectedFile] = useState<any>(null);
  const [showRightPanel, setShowRightPanel] = useState(false);
  const [parsedSequences, setParsedSequences] = useState<SequenceMetadata[]>([]);
  ...
  const handleUploadComplete = (sequences: SequenceMetadata[]) => {
  ...
  return (
    <div className="flex h-screen bg-gray-50 dark:bg-gray-950 overflow-hidden relative">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <TopBar onTogglePanel={() => setShowRightPanel(!showRightPanel)} />
            ...
            <AnimatePresence mode="wait">
              {activeView === 'upload' && (
                <AnimatedPage key="upload" animation="slide-up">
                  ...
                  >
                    <h1 className="text-gray-900 dark:text-white mb-2">Upload FASTA Files</h1>
                    <p className="text-gray-600 dark:text-gray-400">Analyze DNA sequences and generate insights</p>
                  </motion.div>
              ...
              {activeView === 'recent' && (
                <AnimatedPage key="recent" animation="slide-up">
                  <motion.div
                    ...
                    transition={{ delay: 0.1 }}
                  >
                    <h1 className="text-gray-900 dark:text-white mb-2">Recent Uploads</h1>
                    <p className="text-gray-600 dark:text-gray-400">View and manage your uploaded sequences</p>
                  </motion.div>
                  <RecentUploads onSelectFile={setSelectedFile} setActiveView={setActiveView} />
                </AnimatedPage>
              )}
              ...
              {activeView === 'metadata' && (
                <AnimatedPage key="metadata" animation="slide-up">
                  ...
                  >
                    <div>
                      <h1 className="text-gray-900 dark:text-white mb-2">Metadata Dashboard</h1>
                      <p className="text-gray-600 dark:text-gray-400">Detailed biological analysis of your sequences</p>
                    </div>
