...
export interface QualityPrediction {
  ...
  issues: Array<{
    type: 'high_at_content' | 'low_complexity' | 'contamination' | 'sequencing_error';
...
export interface IntelligentReport {
  ...
  recommendations: string[];
  comparisonToKnown: string;
  experimentalSuggestions: string[];
}
...
function getRuleBasedResponse(message: string, context: any): string {
  ...
  if (lowerMessage.includes('orf') || lowerMessage.includes('open reading frame')) {
    return `An Open Reading Frame (ORF) is a continuous stretch of DNA codons that begins with a start codon (ATG) and ends with a stop codon (TAA, TAG, or TGA). ORFs are potential protein-coding regions. ${context.sequences ? `Your sequences contain ${context.sequences.reduce((sum: number, s: SequenceMetadata) => sum + s.orfs.length, 0)} total ORFs.` : ''}`;
  }
