...
<!DOCTYPE html>
  ...
  <meta charset="UTF-8">
  ${config.title || 'Symbio-NLM Sequence Analysis Report'}</title>
  ...
  <div class="header">
    ...
    ${config.title || 'DNA Sequence Analysis Report'}</h1>
    <div class="meta">
  ...
  <div class="section">
    ...
    <table class="nucleotide-table">
          ...
          Average Sequence Length</td>
          ${stats.avgLength} bp</td>
        </tr>
        
          Longest Sequence</td>
          ${stats.longestSequence.toLocaleString()} bp</td>
        </tr>
        
          Shortest Sequence</td>
          ${stats.shortestSequence.toLocaleString()} bp</td>
        </tr>
        
          Total Base Pairs</td>
          ${stats.totalLength.toLocaleString()} bp</td>
        </tr>
      </tbody>
    </table>
  </div>
  ...
  <div class="section">
    ...
    <table class="nucleotide-table">
          ...
          Adenine (A)</td>
          ${stats.nucleotideDistribution.A}%</td>
        </tr>
        
          Thymine (T)</td>
          ${stats.nucleotideDistribution.T}%</td>
        </tr>
        
          Guanine (G)</td>
          ${stats.nucleotideDistribution.G}%</td>
        </tr>
        
          Cytosine (C)</td>
          ${stats.nucleotideDistribution.C}%</td>
        </tr>
      </tbody>
    </table>
  </div>
      ...
      ${sequences.slice(0, 10).map((seq, index) => `
        <div class="sequence-item">
          ...
          <div class="sequence-details">
            Length:</strong> ${seq.length} bp</div>
            GC Content:</strong> ${seq.gcContent}%</div>
            ORFs Found:</strong> ${seq.orfs.length}</div>
          </div>
          ${config.includeORFDetails && seq.orfs.length > 0 ? `
            <div style="margin-top: 10px; font-size: 13px; color: #6b7280;">
              ORF Details:</strong> ${seq.orfs.map(orf => `${orf.length}bp (${orf.start}-${orf.end})`).join(', ')}
            </div>
          ` : ''}
        </div>
      `).join('')}
